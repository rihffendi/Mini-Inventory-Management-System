...
<div class="col-lg-12 mb-4">
	<div class="card card-small">
		<form id="categoryForm" action="{{route('category.sub-categories.update', $all->id)}}" method="post">
			@csrf
		        ...
		        <div class="col-sm-12 col-md-12">
		            <div class="form-group pb-2">
		              <div class="input-group mb-2">
		                ...
		                <input type="text" class="form-control" placeholder="Category Name" aria-label="Category Name" aria-describedby="basic-addon1"name="name" value="{{$all->subcategoryName}}"> </div>
		            </div>
						...
						<div class="input-group mb-3">
							...
							<select name="category" id="inputState" class="form-control"  aria-label="status" aria-describedby="basic-addon1" >{{-- 
							  ...
							  @foreach($category as $categories)	
							  	<option value="{{$categories->id}}" {{$all->category_id == $categories->id  ? 'selected' : ''}}>{{$categories->name}}</option>
							  @endforeach
						    </select>
						</div>
					</div>
					<div class="form-group">
						<div class="input-group mb-3">
							...
							<select name="status" id="inputState2" class="form-control"  aria-label="status" aria-describedby="basic-addon1" >{{-- 
							  ...
							  <option value="1" {{$all->status == 1 ? 'selected' : ''}}>Active</option>
							  <option value="0" {{$all->status == 0 ? 'selected' : ''}}>Inactive</option>
						    </select>
						</div>
					</div>
	          	  	<div class="form-group">
		              <div class="input-group mb-3">
		                ...
		                <textarea class="form-control" placeholder="Category Details" aria-label="Category Details" aria-describedby="basic-addon1" name="details">{{$all->details}}</textarea></div>
		            </div>
